...
proto.login = function (account, password) {
  ...
  .then(function(users) {
    if (_.isEmpty(users)) {
      throw new Error("The email address you entered is incorrect");
    }
    return users;
  })
  .then(function (users) {
    if (tryLoginTimes > 0) {
      ...
      .then(function (loginErrorTimes) {
        if (loginErrorTimes > tryLoginTimes) {
          throw new Error(`Login attempts exceeded and the account is locked`);
        }
        return users;
      });
    } else {
...
proto.checkRegisterCode = function (email, token) {
  ...
  .then(function (u) {
    if (u) {
      throw new Error(`"${email}"Already registered, please replace the mailbox registration`);
    }
  })
  .then(function () {
    ...
    .then(function (storageToken) {
      ...
      if (!_.eq(token, storageToken)) {
        ...
        throw new Error(`The verification code you entered is not correct. Please re-enter it`);
      }
      return storageToken;
    })
  })
...
proto.changePassword = function (uid, oldPassword, newPassword) {
  if (!_.isString(newPassword) || newPassword.length < 6) {
    return Promise.reject(new Error("Please enter a new password with a length of 6 to 20 digits"));
  }
