  ...
  return Promise.all([
    ...
    .then(function () {
      if (fileType == "application/zip") {
        ...
        throw new Error("The uploaded file is not in the correct format");
      }
    })
  ])
...
proto.rollbackPackage = function (deploymentVersionId, targetLabel, rollbackUid) {
  ...
  .then(function(deploymentsVersions){
    if (!deploymentsVersions) {
      throw new Error("You have not released a version yet");
    }
    ...
    .spread(function (currentPackageInfo, rollbackPackageInfos){
      ...
      throw new Error("There is no version available for rollback");
    })
